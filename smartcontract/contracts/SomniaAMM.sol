...
contract SomniaAMM is Ownable, ReentrancyGuard {
    ...
    modifier validTokens(address token0, address token1) {
        ...
        require(
            whitelistedTokens[token0] && whitelistedTokens[token1],
            "SomniaAMM: TOKEN_NOT_WHITELISTED"
        );
        _;
    }
    ...
    modifier poolExists(address token0, address token1) {
        require(
            pools[token0][token1].exists || pools[token1][token0].exists,
            "SomniaAMM: POOL_NOT_EXISTS"
        );
        _;
    }
    ...
    function createPool(
        address token0,
        address token1
    ) external validTokens(token0, token1) returns (bool) {
        require(
            !pools[token0][token1].exists && !pools[token1][token0].exists,
            "SomniaAMM: POOL_EXISTS"
        );
    ...
    {
        Pool storage pool = pools[token0][token1].exists
            ? pools[token0][token1]
            : pools[token1][token0];
        ...
        if (pool.totalSupply == 0) {
            ...
            uint256 amount1Optimal = (amount0Desired * pool.reserve1) /
                pool.reserve0;
            if (amount1Optimal <= amount1Desired) {
                require(
                    amount1Optimal >= amount1Min,
                    "SomniaAMM: INSUFFICIENT_AMOUNT_1"
                );
                amount0 = amount0Desired;
                amount1 = amount1Optimal;
            } else {
                uint256 amount0Optimal = (amount1Desired * pool.reserve0) /
                    pool.reserve1;
                require(
                    amount0Optimal <= amount0Desired,
                    "SomniaAMM: INSUFFICIENT_AMOUNT_0"
                );
                require(
        ...
        returns (uint256 amountOut)
    {
        ...
        require(
            ...
            "SomniaAMM: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    ...
    function getAmountOut(
        ...
        uint256 reserveIn = pool.token0 == tokenIn
            ? pool.reserve0
            : pool.reserve1;
        uint256 reserveOut = pool.token0 == tokenIn
            ? pool.reserve1
            : pool.reserve0;

        uint256 amountInWithFee = amountIn *
            (FEE_DENOMINATOR - TRADING_FEE - LIQUIDITY_FEE);
    ...
    function getStats()
        external
        view
        returns (uint256 _totalVolume, uint256 _totalFees)
    {
