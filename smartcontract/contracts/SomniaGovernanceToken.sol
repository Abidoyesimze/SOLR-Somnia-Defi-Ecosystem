...
contract SomniaGovernanceToken is ERC20, Ownable, Pausable, ReentrancyGuard, ERC20Permit, ERC20Votes {
    ...
    event GovernanceTokenMinted(address indexed to, uint256 amount, string reason);
    ...
    ) ERC20("Somnia Governance", "SOMG") ERC20Permit("Somnia Governance") Ownable(initialOwner) {
    ...
    function mintForStaking(address to, uint256 amount) public onlyAuthorizedMinter whenNotPaused nonReentrant {
        require(mintingEnabled, "Minting is disabled");
        require(totalSupply() + amount <= maxSupply, "Exceeds max supply");
        require(to != address(0), "Cannot mint to zero address");
