...
contract SomniaStaking is Ownable, ReentrancyGuard {
    ...
    // Events
    event PoolCreated(address indexed stakingToken, address indexed rewardToken, uint256 rewardRate);
    event Staked(address indexed user, address indexed stakingToken, uint256 amount, uint256 tier);
    event Unstaked(address indexed user, address indexed stakingToken, uint256 amount, uint256 penalty);
    ...
    event TierAdded(address indexed stakingToken, string tierName, uint256 rewardMultiplier);

    // Modifiers
    modifier poolExists(address stakingToken) {
        require(stakingPools[stakingToken].isActive, "SomniaStaking: POOL_NOT_EXISTS");
        _;
    }
    ...
    modifier validTokens(address stakingToken, address rewardToken) {
        require(whitelistedStakingTokens[stakingToken], "SomniaStaking: STAKING_TOKEN_NOT_WHITELISTED");
        require(whitelistedRewardTokens[rewardToken], "SomniaStaking: REWARD_TOKEN_NOT_WHITELISTED");
        _;
    }

    modifier hasStake(address stakingToken) {
        require(userStakes[msg.sender][stakingToken].isActive, "SomniaStaking: NO_STAKE");
        _;
    }
    ...
    function createPool(
        ...
        require(!stakingPools[stakingToken].isActive, "SomniaStaking: POOL_EXISTS");
        require(rewardRate <= MAX_REWARD_RATE, "SomniaStaking: REWARD_RATE_TOO_HIGH");
        require(minStakeDuration <= maxStakeDuration, "SomniaStaking: INVALID_DURATION");
    ...
    function addStakingTier(
        ...
        require(rewardMultiplier <= 20000, "SomniaStaking: MULTIPLIER_TOO_HIGH"); // Max 2x
        require(earlyWithdrawalPenalty <= 5000, "SomniaStaking: PENALTY_TOO_HIGH"); // Max 50%
     ...
     * @dev Stake tokens
     */
    function stake(address stakingToken, uint256 amount, uint256 tierIndex) external poolExists(stakingToken) validAmount(amount) nonReentrant {
        ...
        require(tierIndex < stakingTiers[stakingToken].length, "SomniaStaking: INVALID_TIER");
        
        StakingTier memory tier = stakingTiers[stakingToken][tierIndex];
        require(amount >= tier.minStake && amount <= tier.maxStake, "SomniaStaking: AMOUNT_OUT_OF_TIER_RANGE");
     ...
     * @dev Unstake tokens
     */
    function unstake(address stakingToken, uint256 amount) external poolExists(stakingToken) hasStake(stakingToken) nonReentrant {
        StakingPool storage pool = stakingPools[stakingToken];
        UserStake storage userStake = userStakes[msg.sender][stakingToken];
        
        require(amount <= userStake.amount, "SomniaStaking: INSUFFICIENT_STAKE");
        require(amount > 0, "SomniaStaking: INVALID_AMOUNT");
        ...
        for (uint256 i = 0; i < stakingTiers[stakingToken].length; i++) {
            ...
            if (userStake.amount >= tier.minStake && userStake.amount <= tier.maxStake) {
     ...
     */
    function claimRewards(address stakingToken) external poolExists(stakingToken) hasStake(stakingToken) nonReentrant {
        StakingPool storage pool = stakingPools[stakingToken];
        UserStake storage userStake = userStakes[msg.sender][stakingToken];
    ...
    function _updateRewards(address stakingToken) internal {
        ...
        uint256 rewardsPerToken = (timeElapsed * pool.rewardRate * REWARD_PRECISION) / (SECONDS_PER_YEAR * 10000);
        ...
        UserStake storage userStake = userStakes[msg.sender][stakingToken];
        if (userStake.isActive) {
            uint256 rewardPerToken = pool.rewardPerTokenStored - userStake.rewardDebt;
            uint256 rewards = (userStake.amount * rewardPerToken) / REWARD_PRECISION;
            
            // Apply tier multiplier
            uint256 tierMultiplier = _getTierMultiplier(stakingToken, userStake.amount);
     ...
     */
    function _getTierMultiplier(address stakingToken, uint256 stakeAmount) internal view returns (uint256) {
        for (uint256 i = 0; i < stakingTiers[stakingToken].length; i++) {
            StakingTier memory tier = stakingTiers[stakingToken][i];
    ...
    function getPendingRewards(address user, address stakingToken) external view returns (uint256) {
        ...
        uint256 rewards = (userStake.amount * rewardPerTokenDelta) / REWARD_PRECISION;
     ...
     */
    function getUserStake(address user, address stakingToken) external view returns (UserStake memory) {
     ...
     */
    function getPool(address stakingToken) external view returns (StakingPool memory) {
     ...
     */
    function getStakingTiers(address stakingToken) external view returns (StakingTier[] memory) {
     ...
     */
    function getProtocolStats() external view returns (uint256 _totalStaked, uint256 _totalRewardsDistributed) {
     ...
     */
    function removeFromWhitelist(address token, bool isStakingToken) external onlyOwner {
        if (isStakingToken) {
     ...
     */
    function updatePoolRewardRate(address stakingToken, uint256 newRewardRate) external onlyOwner poolExists(stakingToken) {
        require(newRewardRate <= MAX_REWARD_RATE, "SomniaStaking: REWARD_RATE_TOO_HIGH");
     ...
     */
    function pausePool(address stakingToken) external onlyOwner poolExists(stakingToken) {
    ...
    function resumePool(address stakingToken) external onlyOwner {
        require(stakingPools[stakingToken].stakingToken != address(0), "SomniaStaking: POOL_NOT_EXISTS");
