...
contract SomniaLending is Ownable, ReentrancyGuard {
    ...
    event Liquidate(address indexed liquidator, address indexed user, address indexed collateralToken, uint256 collateralAmount, uint256 debtAmount);
    event InterestAccrued(address indexed token, uint256 supplyRate, uint256 borrowRate);
    ...
    modifier validToken(address token) {
        require(whitelistedTokens[token], "SomniaLending: TOKEN_NOT_WHITELISTED");
        _;
    }
     ...
     */
    function createMarket(address token, uint256 collateralFactor) external onlyOwner validToken(token) {
        require(!markets[token].isActive, "SomniaLending: MARKET_EXISTS");
        require(collateralFactor <= 9000, "SomniaLending: INVALID_COLLATERAL_FACTOR"); // Max 90%
     ...
     */
    function supply(address token, uint256 amount) external marketExists(token) validAmount(amount) nonReentrant {
        ...
        // Calculate exchange rate and mint shares
     ...
     */
    function withdraw(address token, uint256 amount) external marketExists(token) validAmount(amount) nonReentrant {
        Market storage market = markets[token];
        UserPosition storage position = userPositions[msg.sender][token];
        ...
        uint256 shares = (amount * 1e18) / market.exchangeRate;
        require(shares <= position.supplied, "SomniaLending: INSUFFICIENT_BALANCE");
        
        // Check collateral requirements
        require(_isCollateralSufficient(msg.sender), "SomniaLending: INSUFFICIENT_COLLATERAL");
     ...
     */
    function borrow(address token, uint256 amount) external marketExists(token) validAmount(amount) nonReentrant {
        Market storage market = markets[token];
        UserPosition storage position = userPositions[msg.sender][token];
        
        require(market.totalSupply >= amount, "SomniaLending: INSUFFICIENT_LIQUIDITY");
        require(_isCollateralSufficient(msg.sender), "SomniaLending: INSUFFICIENT_COLLATERAL");
     ...
     */
    function repay(address token, uint256 amount) external marketExists(token) validAmount(amount) nonReentrant {
        Market storage market = markets[token];
        UserPosition storage position = userPositions[msg.sender][token];
     ...
     */
    function liquidate(address user, address collateralToken, address debtToken, uint256 debtAmount) external marketExists(collateralToken) marketExists(debtToken) nonReentrant {
        require(user != msg.sender, "SomniaLending: CANNOT_LIQUIDATE_SELF");
        require(!_isCollateralSufficient(user), "SomniaLending: POSITION_HEALTHY");
        
        UserPosition storage collateralPosition = userPositions[user][collateralToken];
        UserPosition storage debtPosition = userPositions[user][debtToken];
        
        require(collateralPosition.supplied > 0, "SomniaLending: NO_COLLATERAL");
        require(debtPosition.borrowed > 0, "SomniaLending: NO_DEBT");
        
        // Calculate liquidation amounts
        uint256 collateralAmount = _calculateLiquidationAmount(user, collateralToken, debtToken, debtAmount);
        ...
        if (collateralToken != address(0)) {
            IERC20(collateralToken).safeTransfer(msg.sender, totalCollateralToTransfer);
        }
        
        emit Liquidate(msg.sender, user, collateralToken, collateralAmount, debtAmount);
    }
    ...
    function _accrueInterest(address token) internal {
        ...
        if (timeElapsed > 0) {
            // Calculate interest
            uint256 supplyInterest = (market.totalSupply * market.supplyRate * timeElapsed) / (SECONDS_PER_YEAR * 10000);
            uint256 borrowInterest = (market.totalBorrow * market.borrowRate * timeElapsed) / (SECONDS_PER_YEAR * 10000);
    ...
    function _isCollateralSufficient(address user) internal view returns (bool) {
        ...
        uint256 collateralRatio = (totalCollateralValue * 10000) / totalBorrowValue;
     ...
     */
    function _calculateLiquidationAmount(address user, address collateralToken, address debtToken, uint256 debtAmount) internal view returns (uint256) {
     ...
     */
    function getProtocolTotals() external view returns (uint256 _totalCollateral, uint256 _totalBorrowed) {
     ...
     */
    function updateMarket(address token, uint256 collateralFactor) external onlyOwner marketExists(token) {
        require(collateralFactor <= 9000, "SomniaLending: INVALID_COLLATERAL_FACTOR");
